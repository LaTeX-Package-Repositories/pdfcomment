\DocumentMetadata{tagging=on,pdfstandard=ua-2,uncompress}
...
\marginnote{\par% tagging doesn't like textblock directly inside marginnote ...
 \begin{textblock*}{1cm}(7cm,14.5cm)
...
\end{textblock*}}
{\small
\marginnote{\par\begin{textblock*}{2cm}(6cm,11.9cm){($x_1, y_1$)}\end{textblock*}}
\marginnote{\par\begin{textblock*}{2cm}(3cm,13.3cm){($x_2, y_2$)}\end{textblock*}}
\marginnote{\par\begin{textblock*}{2cm}(5.6cm,13.6cm){($x_3, y_3$)}\end{textblock*}}
}
...
\begin{tikzpicture}[alt=a diagram]
...
\end{tikzpicture}
\hspace{1cm}
\begin{tikzpicture}[alt=statistik sausages]
