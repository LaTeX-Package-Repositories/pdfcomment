...
Not all commands work already properly in a such a tagged PDF. 

Known restrictions are:
\begin{itemize}
\item \cs{pdfmarkupcomment} uses currently the soul package to get the dimension of the annotation. The soul package is not compatible with the tagging, and so tagging is suspended in the argument of the command.
\item \cs{pdfmarkupcomment} is currently not tagged.
