...
The original author of the package, Josef Kleber, died some time ago and with version 3.0 the package has a new maintainer: Ulrike Fischer. 
...
\textbf{New in version 3.0:} If the PDF management is loaded the option is ignored.
...
\textbf{New in version 3.0:} If the PDF management is loaded and the new code is used this key is ignored. The code from version 2.0 is always used. 
...
\textbf{Note:} With the new code used if PDF management is loaded, options are always set locally and the option \styleoption{local} is ignored.
...
\textbf{Note:} With the new code used if the PDF management is loaded the option can be also used in the optional argument of the various comment commands and then change the way they are added to list of pdf comments. An empty value suppresses the entry for this comment. 
