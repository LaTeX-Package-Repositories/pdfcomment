...
The original author of the package died some time ago and with version 3.0 the package has a new maintainer: Ulrike Fischer. 
...
\item If the PDF management is loaded, either with \verb+\RequirePackage{pdfmanagement}+ or with \cs{DocumentMetadata}, then the annotations are created with the code from the PDF management and (basic) tagging support has added. 
    ...
    This is big change and quite probably not everything will work from the start. 
...
The option is ignored if the PDF management is used. The backend has to be declared with verb+backend=dvipdfmx+ when loading the PDF management.
